...
async function WomenProducts({ searchParams }: { searchParams: any }) {
  try {
    ...
    if (!productsData || !Array.isArray(productsData)) {
      return (
        <ErrorPage
          title="Error al cargar productos de mujer"
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        ...
        {sortedProducts.length > 0 ? (
          ...
          <NoProductsFound
            title="No hay productos para mujer"
            description="No se encontraron productos femeninos que coincidan con tus filtros. Intenta ajustar los criterios de búsqueda o explora otras categorías."
    ...
    return (
      <ErrorPage
        title="Error al cargar la colección de mujer"
...
export default function WomenPage({ searchParams }: { searchParams: any }) {
  return (
    <Suspense fallback={<CategoryLoadingScreen category="Mujer" />}>
